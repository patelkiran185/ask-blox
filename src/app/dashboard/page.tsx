...
const CONTAINER_WIDTH = 1000; // Further increased width for the visualization area in pixels
const CONTAINER_HEIGHT = 800; // Further increased height for the visualization area in pixels
...
const initialCategories: Omit<Category, 'x' | 'y' | 'size'>[] = [
  ...
  const generateNonOverlappingPositions = (
    ...
    const centerX = containerWidth / 2; // Center in pixels
    const centerY = containerHeight / 2; // Center in pixels
    const minBufferDistance = 30; // Minimum pixel gap between bubble edges

    const positions: Position[] = [];
    
    categories.forEach((cat) => {
      const currentBubbleDiameter_px = Math.max(100, cat.score * 1.7 + 60); // Diameter in pixels
      const currentBubbleRadius_px = currentBubbleDiameter_px / 2;

      let x_px = centerX + (Math.random() - 0.5) * (containerWidth * 0.4); // Random initial position closer to center
      let y_px = centerY + (Math.random() - 0.5) * (containerHeight * 0.4);

      let attempts = 0;
      const maxPlacementAttempts = 5000; // Max attempts to place a single bubble
      ...
      while (hasOverlap && attempts < maxPlacementAttempts) {
        ...
        for (const pos of positions) {
          ...
          const minRequiredDistanceBetweenCenters_px = currentBubbleRadius_px + otherBubbleRadius_px + minBufferDistance; // Sum of radii + buffer

          if (distanceBetweenCenters_px < minRequiredDistanceBetweenCenters_px) {
            hasOverlap = true;

            // Calculate overlap amount and push-away vector
            const overlap = minRequiredDistanceBetweenCenters_px - distanceBetweenCenters_px;
            const angle = Math.atan2(dy, dx); // Angle from other bubble to current bubble

            dx_total += Math.cos(angle) * overlap * 0.5; // Push away gently
            dy_total += Math.sin(angle) * overlap * 0.5;
          }
        }
        ...
        if (hasOverlap) {
          ...
          // Clamp to container bounds after moving (with padding)
          const padding_px = 60;
          x_px = Math.max(currentBubbleRadius_px + padding_px, Math.min(containerWidth - currentBubbleRadius_px - padding_px, x_px));
          y_px = Math.max(currentBubbleRadius_px + padding_px, Math.min(containerHeight - currentBubbleRadius_px - padding_px, y_px));
        }
      ...
      if (hasOverlap) {
        console.warn(`Failed to find non-overlapping position for ${cat.name} after ${attempts} attempts. Using last best position.`);
      }

      // Convert pixel coordinates to percentage for CSS `top` and `left`
      positions.push({
        x: (x_px / containerWidth) * 100,
        y: (y_px / containerHeight) * 100,
        size: currentBubbleDiameter_px, // Keep size in pixels for JSX `width` and `height`
      });
    });
  ...
  useEffect(() => {
    const positions = generateNonOverlappingPositions(initialCategories, CONTAINER_WIDTH, CONTAINER_HEIGHT);
  ...
  const getPercentageFontSize = (bubbleSize: number) => {
    const minBubbleSize = 80; // Smallest bubble size
    const maxBubbleSize = 250; // Increased max bubble size for font scaling
    const minFontSize = 16; // Min font size for percentage
    const maxFontSize = 24; // Max font size for percentage

    if (bubbleSize <= minBubbleSize) return minFontSize;
    if (bubbleSize >= maxBubbleSize) return maxFontSize;

    const ratio = (bubbleSize - minBubbleSize) / (maxBubbleSize - minBubbleSize);
  ...
  const getNameFontSize = (bubbleSize: number) => {
    ...
    const maxBubbleSize = 250; // Increased max bubble size for font scaling
    const minFontSize = 10; // Min font size for name
    const maxFontSize = 14; // Max font size for name
  ...
  return (
    ...
    <div className="relative w-full min-h-[600px] bg-black overflow-hidden">
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        <div className="relative" style={{ width: `${CONTAINER_WIDTH}px`, height: `${CONTAINER_HEIGHT}px` }}> {/* Dynamically sized container */}
          {/* Animated bubbles */}
          <AnimatePresence>
                ...
                <div className="text-center drop-shadow-2xl">
                  ...
                  >
                    {category.score}%
                      ...
                      <div 
                        className="text-sm opacity-95 font-semibold backdrop-blur-sm"
                      >
                        {category.name}
                  </div>
                </div>
        ...
        {selectedCategory && (
          <div className="fixed inset-0 flex items-center justify-center z-[100]">
            ...
            <motion.div
              ...
              className="relative w-[90vw] max-w-[400px] bg-white/10 backdrop-blur-lg rounded-xl p-6 text-white shadow-2xl border border-white/20"
            >
                      ...
                      <div className="space-y-2">
                        {lackingSkills.map((skill, idx) => (
                          <React.Fragment key={idx}>
                            <div
                              onClick={() => {
                                if (expandedSkill === skill.name) {
                                  setExpandedSkill(null); // Hide if already expanded
                                  setSelectedSkill(null);
                                } else {
                                  setExpandedSkill(skill.name); // Show for this skill
                                  setSelectedSkill(skill.name);
                                  setPopupView('main'); // Ensure we are in the main view
                                }
                              }}
                            ...
                            {expandedSkill === skill.name && selectedSkill === skill.name && ( // Render buttons only if this skill is expanded
                              <div className="flex gap-3 mt-2">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation(); // Prevent re-triggering parent div click
                                    setPopupView('resources');
                                    setFetchedRecommendations(null); // Reset recommendations while fetching
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    setFetchedStrategies(null); // Reset strategies while fetching
                                    fetchStrategies(selectedSkill).then(strategies => {
                                      setFetchedStrategies(strategies);
                                    });
                                  }}
              ...
              {popupView === 'resources' && fetchedRecommendations && (
